...
# If row 43 is incorrect, filter by condition 
...
######  EIGEN() ########
### --- EIGEN DECOMPOSITION ---
eigen.Y_dec <- eigen(H.Y, symmetric = TRUE)
eigen.X_dec <- eigen(H.X, symmetric = TRUE)

# Select number of components based on 99% variance explained
L <- which(cumsum(eigen.Y_dec$values[which(eigen.Y_dec$values > 0)]) / sum(eigen.Y_dec$values[which(eigen.Y_dec$values > 0)]) > 0.99)[1]
H <- which(cumsum(eigen.X_dec$values[which(eigen.X_dec$values > 0)]) / sum(eigen.X_dec$values[which(eigen.X_dec$values > 0)]) > 0.99)[1]

# Extract the eigenvectors corresponding to the selected components
phi.Y <- eigen.Y_dec$vectors[, 1:L]
phi.X <- eigen.X_dec$vectors[, 1:H]
...
# --- Construct Regression Matrix X from chi ---
x <- matrix(NA, N, q * H)
colnames(x) <- paste0("h.", sort(rep(c(1:H), q)), "_", rep(x.names, H))

for (h in seq_len(H)) {
  x[, c((q * (h - 1) + 1):(q * h))] <- chi[, , h]
}
...
# --- Determine the cutoff points for 99% explained variability ---
L_cutoff <- which(explained_var_Y[1, ] >= 0.99)[1]
H_cutoff <- which(explained_var_X[1, ] >= 0.99)[1]

cat("Number of Basis Functions for Y (L) capturing 99% variability:", L_cutoff, "\n")
cat("Number of Basis Functions for X (H) capturing 99% variability:", H_cutoff, "\n")
...
B_vector <- model$B[-q, , 2, 1]
B_matrix <- matrix(B_vector, nrow = H, ncol = L)
...
spatial_effects_full <- phi.X %*% B_matrix %*% t(phi.Y)
...
for (loc in colnames(spatial_effects_df)[-(1:2)]) {
  ...
  heatm <- ggplot() +
    ...
    labs(title = paste("Spatial Effect of Precipitation on River Flow at", loc),
         x = "Longitude", y = "Latitude", fill = "Effect") +
    theme_minimal() +
    theme(
...
train_pipeline <- function(q_train_data, prec_train_data) {
  ...
  for(j in seq_len(p)) {
    ...
    for(i in seq_len(N_q)) {
      ...
      if(unique_x1 > 1 && unique_x2 > 1) {
        k_val <- 25
        model_i <- gam(y ~ s(x1, x2, k = k_val), data = data_temp)
        models.plot[[y.names[j]]][[i]] <- list(model = model_i)
  ...
  eigen.Y_dec <- eigen(H.Y_mat, symmetric = TRUE)
  eigen.X_dec <- eigen(H.X_mat, symmetric = TRUE)
  
  # Select number of components based on 99% variance explained
  L <- which(cumsum(eigen.Y_dec$values[which(eigen.Y_dec$values > 0)]) / sum(eigen.Y_dec$values[which(eigen.Y_dec$values > 0)]) > 0.99)[1]
  H_basis <- which(cumsum(eigen.X_dec$values[which(eigen.X_dec$values > 0)]) / sum(eigen.X_dec$values[which(eigen.X_dec$values > 0)]) > 0.99)[1]
  
  # Extract the eigenvectors corresponding to the selected components
  phi.Y <- eigen.Y_dec$vectors[, 1:L]
  phi.X <- eigen.X_dec$vectors[, 1:H_basis]
