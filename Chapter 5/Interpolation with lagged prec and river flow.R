...
x_lag <- rbind(rep(NA, q * H), x_now[-N, ])  # shift down by one row
...
# Display ranges to check for correctness
range(H.Y) # -0.005182528  0.196869191
range(H.X) # -3.186749e-06  7.769790e-03
...
# Initial gamma_lag from last day of training
...
for (t in 1:N) {
  ...
  if (t > 1) {
    chi_t_minus_1 <- chi_new[1, t - 1, ]
  } else {
    chi_t_minus_1 <- chi_t  # fallback if t == 1
  }
  
  # Use only lagged chi and gamma
  X_reg_row <- c(
    1,
    chi_t,
    chi_t_minus_1,
...
# Step 1: AR(1) correction (same structure as before)
residuals_2024 <- matrix(0, nrow = N, ncol = L)
for (l in seq_len(L)) {
  for (t in 3:N) {  # <-- start from 3, not 2
...
# Step 2: Reconstruct Q (still in log1p scale)
Q_pred_log <- gamma_pred_corrected %*% t(phi.Y) +
  matrix(rep(Y.mean, each = N), nrow = N, byrow = FALSE)

# Step 3: Re-add station means and back-transform
...
Q_pred_real_ar1[Q_pred_real_ar1 < 0] <- 0
...
saveRDS(phi.Y, "phi_Y.rds")
saveRDS(Y.mean, "Y_mean.rds")
saveRDS(B_matrix, "B_matrix.rds")
saveRDS(phi, "phi_ar1.rds")
saveRDS(gamma, "gamma_train.rds")
saveRDS(mean_logQ_vec, "mean_logQ_vec.rds")
