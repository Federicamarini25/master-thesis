...
# STEP 4: Create expanded bounding box from 
...
cat("Mean Squared Error (MSE):", MSE, "\n") #  in this way: 0.1355082 
# without data$y <- log1p(data$y): 2.368187 
...
# 1. Combine residuals and location names
residuals_df <- data.frame(
  location_name = all_q_data_imputed$location_name,
  date = all_q_data_imputed$data,
  residual = residuals[, 1]  # First gamma component
)

# 2. Sort by location and date (important for ACF!)
residuals_df <- residuals_df[order(residuals_df$location_name, residuals_df$date), ]

# 3. Get unique locations
locations <- unique(residuals_df$location_name)

# 4. Generate ACF plots for each location
acf_list <- list()
for (loc in locations) {
  resid_loc <- residuals_df$residual[residuals_df$location_name == loc]
  
  # Only compute ACF if we have enough points
  if (length(resid_loc) > 10) {
    p <- ggAcf(resid_loc, lag.max = 30) +
      ggtitle(paste("ACF -", loc)) +
      theme_minimal()
    acf_list[[loc]] <- p
  }
}

# 5. Display 6 plots per page
n_per_page <- 6
n_pages <- ceiling(length(acf_list) / n_per_page)

for (i in seq_len(n_pages)) {
  idx_start <- (i - 1) * n_per_page + 1
  idx_end <- min(i * n_per_page, length(acf_list))
  plots <- acf_list[idx_start:idx_end]
...
H <- dim(chi_new)[3]         # number of basis functions for precipitation
L <- dim(gamma_new)[3]       # number of basis functions for river flow
N <- dim(gamma_new)[2]       # number of units/dates (assumed same across arrays)
...
X_reg_new <- matrix(NA, nrow = N, ncol = H)
colnames(X_reg_new) <- c(paste0("prec_basis_", 1:H))
# Fill in the precipitation scores.
for (h in seq_len(H)) {
  X_reg_new[, h] <- chi_new[1, , h]
}

# The response matrix Y_reg is taken as the river flow functional scores.
# Its dimensions are N x L.
Y_reg_new <- matrix(NA, nrow = N, ncol = L)
colnames(Y_reg_new) <- paste0("Q_basis_", 1:L)
for (l in seq_len(L)) {
  Y_reg_new[, l] <- gamma_new[1, , l]
}

X_reg_new <- cbind(1, X_reg_new)

gamma_pred_new <- X_reg_new %*% B_matrix  # [N x L]

# Step 1: Compute raw residuals
residuals_2024 <- gamma_new[1,,] - gamma_pred_new  # [N x L]

# Step 2: Apply AR(1) correction to the residuals
resid_corrected <- residuals_2024
for (l in seq_len(ncol(resid_corrected))) {
  for (t in 2:nrow(resid_corrected)) {
    resid_corrected[t, l] <- residuals_2024[t, l] - phi[l] * residuals_2024[t - 1, l]
  }
}

# AR(1) innovation RMSE
rmse_ar1_2024 <- sqrt(mean(resid_corrected^2, na.rm = TRUE))
cat("AR(1)-corrected RMSE for 2024:", rmse_ar1_2024, "\n") # 0.3033453 

# Step 3: Add corrected residuals to the predicted gamma
gamma_pred_corrected <- gamma_pred_new + resid_corrected

# Step 4: Reconstruct predicted Q (still in log1p scale)
Q_pred_log <- gamma_pred_corrected %*% t(phi.Y) +
  matrix(rep(Y.mean, each = N), nrow = N, byrow = FALSE)

# Step 5: Inverse log1p to get actual Q predictions
Q_pred_real_ar1 <- exp(Q_pred_log) - 1  # Final corrected predictions
