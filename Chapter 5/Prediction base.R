...
gamma_sd <- apply(residuals, 2, sd, na.rm = TRUE)  # length L
...
save(B_matrix, H.X, H.Y, new.data, phi.X, phi, phi.Y, station_means, 
     ...
     weight.x, weight.y, gamma_sd, prec_elev, q_elev, file = "functional.Tesi.RData")
...
# PLOT THAT WE'LL SEE IN THE APP ----
imp_func <- impute_missing_data(all_q_data, method ="functional")

filtered_data <- imp_func %>%
  filter(location_name == "Rovana - Galleria di derivazione")
...
predict_river_flow_df <- function(chi, gamma, B_matrix, phi.Y, Y.mean,
  ...
  for (t in 1:N) {
    chi_tm1 <- if (t > 1) chi[1, t - 1, ] else chi[1, t, ]
